...
def cli():
    ...
    delete_parser.add_argument(
        "book_id", type=str, help="ID of the book to delete"
    )

    # Поиск книги
    search_parser = subparsers.add_parser("search", help="Search for books")
    search_parser.add_argument(
        "search_term", type=str, help="Search term (title, author, or year)"
    )
    ...
    change_status_parser = subparsers.add_parser(
        "change_status", help="Change book status"
    )
    change_status_parser.add_argument(
        "book_id", type=str, help="ID of the book"
    )
    change_status_parser.add_argument(
        "new_status", type=str, help='New status ("в наличии" or "выдана")'
    )
